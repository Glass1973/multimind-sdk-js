...
import { Document, builtInTools } from '../src/index';
...
async function comprehensiveDemo() {
  ...
  try {
    ...
    try {
      ...
      console.log('Basic agent failed:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('Advanced fine-tuning failed:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('Advanced RAG failed:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('Model conversion failed:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('Compliance check failed:', (error as Error).message);
    }
    ...
    try {
      const agentResponse = await sdk.runAdvancedAgent(
        "Calculate 15 * 23 and then search for information about quantum computing",
        { context: "mathematical and scientific inquiry" }
      );
      console.log('Advanced Agent Response:', agentResponse);
    } catch (error) {
      console.log('Advanced agent failed:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('Model client system failed:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('Gateway failed:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('Model evaluation failed:', (error as Error).message);
    }
    
    // 12. Model Comparison
    console.log('\n12. ⚖️ Model Comparison');
    try {
      const comparison = await sdk.compareModels(
        ["gpt-3.5-turbo", "claude-3", "mistral"],
        "text-classification",
        "benchmark-dataset"
      );
      console.log('Model comparison:', comparison);
    } catch (error) {
      console.log('Model comparison failed:', (error as Error).message);
    }
