...
async function advancedExample() {
  ...
  try {
    ...
    try {
      const routedModel = await sdk.routeModel(
        "I need to analyze sentiment in customer reviews",
        ['sentiment-analyzer', 'gpt-4', 'bert-base']
      );
      console.log('Routed to model:', routedModel);
    } catch (error) {
      console.log('Model routing failed:', (error as Error).message);
    }
    ...
    */
    ...
    try {
      ...
      console.log('Adapter listing failed:', (error as Error).message);
    }
